...
router.post('/', upload.array('files', 200), async (req, res) => {
  ...
  for (const file of req.files) {
    // Register file in DB — tag with uploader's org and identity
    const info = db.prepare(`
      INSERT INTO excel_files (filename, filepath, org_id, uploaded_by, status)
      VALUES (?, ?, ?, ?, 'pending')
...
// ── Multer error handler — returns clean JSON instead of HTML 500
router.use((err, req, res, next) => {
  if (err?.code === 'LIMIT_FILE_SIZE') {
    return res.status(400).json({ error: 'File too large — maximum size is 50 MB per file' });
  }
