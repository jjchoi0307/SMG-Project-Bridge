...
const { patientIdScope, mergeWhere } = require('../utils/orgScope');
...
router.get('/auths', (req, res) => {
  ...
  if (scope.conditions.length) { conditions.push(...scope.conditions.map(c => c.replace('patient_id', 'a.patient_id'))); params.push(...scope.params); }
  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
