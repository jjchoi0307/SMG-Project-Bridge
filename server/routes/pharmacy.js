...
const { patientIdScope } = require('../utils/orgScope');
...
router.get('/refills/due', (req, res) => {
  ...
  const extra = scope.conditions.length
    ? 'AND ' + scope.conditions.map(c => c.replace('patient_id', 'pr.patient_id')).join(' AND ')
    : '';
  ...
  `).all(parseInt(days), ...scope.params);
  res.json(rows);
});
